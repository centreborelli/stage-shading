...
def est_dans_ellipse(i,j,r) :
    return(rx*(i-i0)**2+ry*(j-j0)**2 < r**2)
def est_au_bord(i,j,r) :
    L = voisins(i,j)
    for voisin in L[0] :
        if est_dans_ellipse(voisin[0],voisin[1],r) :
            return(True)
    for voisin in L[1] :
        if est_dans_ellipse(voisin[0],voisin[1],r) :
            return(True)
...
# path = 'D:/L3/Stage/Fast marching SFS'
# os.chdir(path)
